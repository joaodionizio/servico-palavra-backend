...
public sealed record ConteudoResumoResponse(Guid Id, string Titulo, string Slug, string? Resumo, TipoConteudo Tipo, OrigemConteudo Origem, string Url, string? UrlThumbnail, int? DuracaoMinutos, Guid? CategoriaConteudoId, string? Categoria, bool Destaque, DateTime? PublicadoEm);
...
public sealed record MaterialApoioRequest(string Titulo, string? Descricao, TipoMaterialApoio Tipo, string Url, int? Ordem = null, bool? Ativo = null);
