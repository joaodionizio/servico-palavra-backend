...
public sealed class FavoritoRepository : IFavoritoRepository
{
    ...
    public async Task<IReadOnlyList<Favorito>> ListByUsuarioAsync(Guid usuarioId, CancellationToken cancellationToken = default) =>
        await _db.Favoritos
            .Include(x => x.Conteudo)
            .ThenInclude(x => x.CategoriaConteudo)
